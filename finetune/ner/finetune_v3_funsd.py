...
from utils.dataset import create_sampler, create_loader
from utils.scheduler_factory import create_scheduler
from utils.optim_factory import create_optimizer
from get_aug_image import get_image

data_dir="/mix_FUNSD_H/data"
img_path="/mix_FUNSD_H/training_data/images"
label_path='/mix_FUNSD_H/label.txt'
...
train_words, train_labels, train_boxes, train_images, train_actual_boxes, train_images_path = read_examples_from_file(
    data_dir, mode='train')
...
token_=LayoutLMv3Tokenizer.from_pretrained("microsoft/layoutlmv3-base")
...
def train(model, data_loader, optimizer, epoch, warmup_steps, device, scheduler):
    ...
    for i, (image_path,
        ...
        loss_cls, _= model(text=tokens, bbox=bboxes, attention_mask=attention_masks, image=image,
...
def main(args, config):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        train_stats = train(model, data_loader, optimizer, epoch, warmup_steps, device, lr_scheduler)
        if util.is_main_process():
            ...
            #torch.save(save_obj, os.path.join(args.output_dir, 'funsd_v3_align_%02d.pth' % epoch))
