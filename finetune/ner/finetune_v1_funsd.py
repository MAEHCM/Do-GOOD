...
train_dataset = V2Dataset(Process(train_words, train_boxes, train_labels))
...
def train(model, data_loader, optimizer, epoch, warmup_steps, device, scheduler):
    ...
    for i, (
            ...
            attention_masks,
            bboxes,
            labels,seg_ids) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_cls, _= model(text=tokens, bbox=bboxes, attention_mask=attention_masks, image=None,
...
def main(args, config):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if util.is_main_process():
            ...
            torch.save(save_obj, os.path.join(args.output_dir, 'funsd_v1_%02d.pth' % epoch))
