...
tokenizer_v2=LayoutLMv2Tokenizer.from_pretrained("microsoft/layoutlmv2-base-uncased")
        ...
        for i in range(len(encoded_inputs['labels'])):
            for j in range(len(encoded_inputs['labels'][i])):
                if encoded_inputs['labels'][i][j]!=-100:
                    ...
                    temp_str=tokenizer_v2.convert_ids_to_tokens(temp_num)
                    res_str=temp_str[0]+augmenter.augment(temp_str[1:])[0]
                    res_input_ids=tokenizer_v2.convert_tokens_to_ids(res_str)
