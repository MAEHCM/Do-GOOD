...
tokenize_v1=BertTokenizer.from_pretrained("microsoft/layoutlm-base-uncased")
...
def Process(words, boxes, labels):
    for idx in range(len(words)):
        ...
        '''for i in range(len(word)):
            temp_str=tokenize_v3.convert_ids_to_tokens(word[i])[1:].lower()
            ids=tokenize_v1.convert_tokens_to_ids(temp_str)
            word[i]=ids'''

        box=boxes[idx]
        label=labels[idx]

        word=[tokenize_v1.cls_token_id]+word
        word=word+[tokenize_v1.sep_token_id]

        box.insert(0,[0,0,0,0])
        box.append([1000,1000,1000,1000])
...
def main(args, config):
    ...
    data_test_loader = DataLoader(datasets_test, batch_size=1)
            ...
            if out_label_ids[i, j] != -100:
                if j > 0:
                    if ocr_boxes[i][j].tolist() == ocr_boxes[i][j - 1].tolist():
                        continue
                if ocr_boxes[i][j][1] <= 150:
                    if idx2label[out_label_ids[i][j]][2:] == 'ANSWER' or idx2label[out_label_ids[i][j]][
                                                                         2:] == 'QUESTION' or idx2label[
                                                                                                  out_label_ids[i][j]][
                                                                                              2:] == 'OTHER':
                        gt += 1
    ...
    for i in range(out_label_ids.shape[0]):
        for j in range(out_label_ids.shape[1]):
            if out_label_ids[i, j] != -100:
                if ocr_boxes[i][j][1] > 150:
                    if iob_to_label(idx2label[out_label_ids[i][j]]) == 'QUESTION' or iob_to_label(
                            idx2label[out_label_ids[i][j]]) == 'ANSWER':
...
if __name__ == '__main__':
    ...
    for idx in range(total_img):
        ...
        image.save('FUNSD_output/{}.png'.format(idx))
