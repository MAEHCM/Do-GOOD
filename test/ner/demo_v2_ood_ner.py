...
img = Image.open(img_none_path).convert('RGB')
...
def main(args, config):
    ...
    gt_true=0
    pred=0
    #prec:在所有被预测为O的样本中实际为O的样本的概率
    for i in range(out_label_ids.shape[0]):
        for j in range(out_label_ids.shape[1]):
            if out_label_ids[i, j] != -100:
                if idx2label[preds[i][j]] == 'O':
                    pred += 1
                if idx2label[out_label_ids[i][j]]=='O' and idx2label[preds[i][j]] == 'O':
                    gt_true+=1

    prec=gt_true/pred

    #recall:在实际为'O'的样本中被预测为'O'样本的概率
    gt = 0
    pred_true = 0
    for i in range(out_label_ids.shape[0]):
        for j in range(out_label_ids.shape[1]):
    ...
    other_f1=2*prec*recall/(prec+recall)
    ...
    print('other results:', results_other)
    ################################################################对于中间区域
    ...
    results3 = {
        "precision": precision_score(out_qa_list, preds_qa_list),
        "recall": recall_score(out_qa_list, preds_qa_list),
        "f1": f1_score(out_qa_list, preds_qa_list),
    }

    print('question and answer results:', results3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='/funsd_v2_24.pth')
