...
def main(model):
    ...
    return preds_list, out_label_list, ocr_list, actual_ocr_list, image_paths, token_list,test_words
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='/mnt/disk2/hjb/funsd_v3_align_69.pth')
    ...
    curr_pred, labels, _, boxes, img_paths, tokens,words = main(model)
    ...
    for i in range(30):
        if i==0:
            ...
            prev_pred = curr_pred
        curr_pred, _, _, _, _, _,_ = main(model)
    ...
    print(f"weak_A_num: {weak_he_num}")

    with open('FUNSD_L/weak_A_map', 'w', encoding='utf-8') as w:
        idx = 0
    ...
    with open('FUNSD_L/weak_Q_map', 'w', encoding='utf-8') as w:
    ...
    with open('FUNSD_L/strong_question_map', 'w', encoding='utf-8') as w:
        idx = -1
    ...
    with open('FUNSD_L/strong_answer_map', 'w', encoding='utf-8') as w:
        idx = -1
        for strong_data in strong_map:
            label = strong_data.split('\t')[-1]
            if label[2:] == 'ANSWER':
    ...
    with open('FUNSD_L/weak_other_map', 'w', encoding='utf-8') as w:
        idx = 0
